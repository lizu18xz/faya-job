...
public class MotanSwitcherUtil {
    ...
    public static void main(String[] args) {
        String swatcherName="test";
        //1-注册
        MotanSwitcherUtil.registerSwitcherListener(swatcherName, new SwitcherListener() {
            @Override
            public void onValueChanged(String key, Boolean value) {
                System.out.println("事件触发后回调函数启动:"+key+"==="+value);
            }
        });

        System.out.println("注册后状态:"+MotanSwitcherUtil.isOpen(swatcherName));
        MotanSwitcherUtil.initSwitcher(swatcherName,true);
        System.out.println("初始化后状态:"+MotanSwitcherUtil.isOpen(swatcherName));
        MotanSwitcherUtil.setSwitcherValue(swatcherName,false);
        System.out.println("修改后状态:"+MotanSwitcherUtil.isOpen(swatcherName));
    }
