...
public class StandardThreadExecutor extends ThreadPoolExecutor {
            ...
            public void run() {
                while (true) {
                    if (getQueue().size() > maxThreads) {
                        log.info("{}current thread task size:{} exceed thread pool define size:{}", Constants.LOG_PREFIX,
    ...
    public static class BlockRejectedExecutionHandler implements RejectedExecutionHandler {
        ...
        public void rejectedExecution(Runnable runnable, ThreadPoolExecutor executor) {
            try {
                log.info("current thread task exceed thread pool define size : {}", StandardThreadExecutor.DEFAULT_MAX_THREADS);
